...
pub use shared::runtime::{TaskExecutor, TaskId, TaskState, Task, TaskHandle, KumirRuntime};

use shared::parser::parse;
use shared::types::{Program, Value, Stmt};
...
"#;
    ...
    fn test_arithmetic() {
        ...
        assert_eq!(interpreter.eval("2 + 3").unwrap(), Value::Number(shared::types::Number::I64(5)));
        assert_eq!(interpreter.eval("10 - 4").unwrap(), Value::Number(shared::types::Number::I64(6)));
        assert_eq!(interpreter.eval("3 * 4").unwrap(), Value::Number(shared::types::Number::I64(12)));
...
"#;
    ...
    fn test_builtin_functions() {
        ...
        // Математика
        assert_eq!(interpreter.eval("abs(-5)").unwrap(), Value::Number(shared::types::Number::I64(5)));
        assert_eq!(interpreter.eval("min(3, 7)").unwrap(), Value::Number(shared::types::Number::I64(3)));
        assert_eq!(interpreter.eval("max(3, 7)").unwrap(), Value::Number(shared::types::Number::I64(7)));
        
        // Строки
        assert_eq!(interpreter.eval("длина(\"привет\")").unwrap(), Value::Number(shared::types::Number::I64(6)));
    }

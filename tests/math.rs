use kumir3_corelib::shared::math::MathOperators;
use kumir3_corelib::shared::types::{Value, Number};
use kumir3_corelib::shared::math::MathErr;
use kumir3_corelib::shared::f128::F128;
...
fn array_subtraction_basic() {
	let a = Value::Array(vec![Value::from(1i64), Value::from(2i64), Value::from(2i64), Value::from(3i64)]);
	let b = Value::Array(vec![Value::from(2i64)]);
	let r = MathOperators::sub(a, b, true).unwrap();
	assert_eq!(r, Value::Array(vec![Value::from(1i64), Value::from(2i64), Value::from(3i64)]));
}
...
fn array_subtraction_collision_large_vb() {
	let a = Value::Array(vec![
		Value::from(1i32), Value::from(1.0_f64), Value::from(2i64), Value::from(3i64)
	]);
	let mut vb = Vec::new();
	vb.push(Value::from(99i32));
	for i in 0..9 { vb.push(Value::from(i as i64 + 100)); }
	vb.push(Value::from(1i32));
	let b = Value::Array(vb);
	let r = MathOperators::sub(a, b, true).unwrap();
	assert_eq!(r, Value::Array(vec![Value::from(1.0_f64), Value::from(2i64), Value::from(3i64)]));
}
...
fn array_subtraction_nested_arrays() {
	...
	let r = MathOperators::sub(a, b, true).unwrap();
	assert_eq!(r, Value::Array(vec![Value::Array(vec![Value::from(2i64)]), Value::Array(vec![Value::from(1i64)])]));
}

#[test]
fn array_subtraction_not_present() {
	let a = Value::Array(vec![Value::from(1i64), Value::from(2i64), Value::from(3i64)]);
...
fn array_subtraction_large_mixed_types() {
	let a = Value::Array(vec![Value::from(1i32), Value::from(1.0_f64), Value::from(2i64)]);
	let mut vb = Vec::new();
	for i in 0..10 { vb.push(Value::from(i as i64 + 100)); }
	vb.push(Value::from(1i32));
	let b = Value::Array(vb);
	let r = MathOperators::sub(a, b, true).unwrap();
	assert_eq!(r, Value::Array(vec![Value::from(1.0_f64), Value::from(2i64)]));
}
...
fn round_with_rf_threshold() {
	...
	let r = MathOperators::round(a, Some(Value::from(1i32)), Some(Value::from(6i32)), true).unwrap();
	assert_eq!(r, Value::from(1.2));

	let a = Value::from(1.25);
	let r = MathOperators::round(a, Some(Value::from(1i32)), Some(Value::from(4i32)), true).unwrap();
...
fn round_preserves_f128_type() {
	let a = Value::from(F128::from(1.235f64));
	let r = MathOperators::round(a, Some(Value::from(2i32)), None, true).unwrap();
	assert_eq!(r, Value::from(F128::from(1.24f64)));
}
...
fn string_divide_by_delimiter_returns_array_and_count() {
	...
	match r {
		Value::Pair(left, right) => {
			...
			} else { panic!("left is not array"); }
			if let Value::Number(Number::I64(v)) = *right { assert_eq!(v, 4); }
			else { panic!("right is not I64"); }
		}
...
fn string_multiplication_and_division() {
	...
	assert_eq!(r3, Value::Array(vec![Value::String("ab".to_string()), Value::String("cd".to_string()), Value::String("ef".to_string())]));
	...
	assert_eq!(r4, Value::Array(vec![Value::String("abc".to_string()), Value::String("de".to_string()), Value::String("fg".to_string())]));
}

#[test]
fn round_invalid_rf_errors() {
	let a = Value::from(1.23);
	let e = MathOperators::round(a.clone(), Some(Value::from(1i32)), Some(Value::from(0i32)), true);
	assert!(e.is_err());
...
fn trig_sin_cos_basic() {
    ...
    } else { panic!("Expected F128 result"); }

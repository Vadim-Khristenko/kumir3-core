...
pub fn iso_utc_fn() -> LibFunctionDef {
    LibFunctionDef::new("дата_время_iso_utc")
        .with_aliases(vec![Arc::from("iso_utc"), Arc::from("datetime_utc_iso")])
...
pub fn iso_local_fn() -> LibFunctionDef {
    LibFunctionDef::new("дата_время_iso")
        .with_aliases(vec![Arc::from("iso"), Arc::from("datetime_iso")])
...
pub fn date_ru_fn() -> LibFunctionDef {
    LibFunctionDef::new("текущая_дата_рус")
        .with_aliases(vec![Arc::from("date_ru"), Arc::from("current_date_ru")])
...
pub fn time_str_fn() -> LibFunctionDef {
    LibFunctionDef::new("текущее_время_строка")
        .with_aliases(vec![Arc::from("time_str"), Arc::from("current_time_str")])
...
pub fn from_timestamp_fn() -> LibFunctionDef {
    LibFunctionDef::new("из_штампа_сек")
        .with_aliases(vec![Arc::from("from_timestamp"), Arc::from("from_ts")])
        .with_description("Преобразует секунды от эпохи в ISO-дату UTC")
        .with_param(LibParamDef::value("сек", TypeKind::Int64))
        .returns(TypeKind::String)
        .with_handler(|args| {
...
pub fn to_timestamp_fn() -> LibFunctionDef {
    LibFunctionDef::new("в_штамп_сек")
        .with_aliases(vec![Arc::from("to_timestamp"), Arc::from("ts")])
        .with_description("Преобразует ISO-дату UTC в секунды от эпохи")
        .with_param(LibParamDef::value("iso", TypeKind::String))
...
pub fn split_timestamp_fn() -> LibFunctionDef {
    LibFunctionDef::new("разобрать_штамп_сек")
        .with_aliases(vec![Arc::from("ts_parts"), Arc::from("split_timestamp")])
        .with_description(
            "Разбивает штамп секунд на компоненты (год, месяц, день, час, минута, секунда)",
        )
        .with_param(LibParamDef::value("сек", TypeKind::Int64))
        .returns(TypeKind::Map(
            Box::new(TypeKind::String),
            Box::new(TypeKind::Auto),
        ))
        .with_handler(|args| {
            let secs = expect_number(args, 0, "сек")?;
            let parts = timestamp_to_parts(secs)?;
...
pub fn split_iso_fn() -> LibFunctionDef {
    LibFunctionDef::new("разобрать_iso")
        .with_aliases(vec![Arc::from("iso_parts"), Arc::from("split_iso")])
...
pub fn make_timestamp_fn() -> LibFunctionDef {
    LibFunctionDef::new("создать_штамп")
        .with_aliases(vec![
            Arc::from("make_timestamp"),
            Arc::from("create_timestamp"),
        ])
        ...
        .with_param(LibParamDef::value("день", TypeKind::UInt8))
        .with_param(LibParamDef::value("час", TypeKind::UInt8))
        .with_param(LibParamDef::value("минута", TypeKind::UInt8))
        .with_param(LibParamDef::value("секунда", TypeKind::UInt8))
        .returns(TypeKind::Int64)
        .with_handler(|args| {
...
pub fn make_date_fn() -> LibFunctionDef {
    LibFunctionDef::new("создать_штамп_дата")
        .with_aliases(vec![Arc::from("make_date"), Arc::from("date_to_timestamp")])
...
pub fn weekdays_fn() -> LibFunctionDef {
    LibFunctionDef::new("дни_недели")
        .with_aliases(vec![Arc::from("weekdays"), Arc::from("weekday_names")])
        ...
        .with_handler(|_args| {
            ...
            for (i, name) in WEEKDAY_RU_LONG.iter().enumerate() {
                map.insert(
                    Value::Number(Number::I64((i + 1) as i64)),
...
pub fn weekdays_short_fn() -> LibFunctionDef {
    LibFunctionDef::new("дни_недели_кратко")
        .with_aliases(vec![Arc::from("weekdays_short")])
        .with_description(
            "Возвращает словарь ISO-дней недели (1..7) -> краткие названия (пн, вт...)",
        )
        .returns(TypeKind::Map(
            Box::new(TypeKind::Int64),
            Box::new(TypeKind::String),
...
pub fn months_fn() -> LibFunctionDef {
    LibFunctionDef::new("месяцы")
        .with_aliases(vec![Arc::from("months"), Arc::from("month_names")])
...
pub fn months_short_fn() -> LibFunctionDef {
    LibFunctionDef::new("месяцы_кратко")
        .with_aliases(vec![Arc::from("months_short")])

...
impl RustBlockExecutor {
    ...
    ) -> Result<RustBlockResult, RustCodeBlockError> {
    ...
    fn execute_compiled(
        ...
        let mut file = fs::File::create(&source_path)
            .map_err(|e| RustCodeBlockError::IO(format!("Не удалось создать файл: {}", e)))?;
        file.write_all(full_code.as_bytes())
            .map_err(|e| RustCodeBlockError::IO(format!("Не удалось записать файл: {}", e)))?;
        ...
        let compile_output = Command::new(&rustc)
            ...
            .arg(&source_path)
            .output()
            .map_err(|e| RustCodeBlockError::IO(format!("Не удалось запустить rustc: {}", e)))?;

        if !compile_output.status.success() {
            let stderr = String::from_utf8_lossy(&compile_output.stderr);
            return Err(RustCodeBlockError::CompileError(format!(
        ...
        let run_output = Command::new(&output_path).output().map_err(|e| {
            RustCodeBlockError::IO(format!("Не удалось выполнить Rust-блок: {}", e))
        })?;
    ...
    fn execute_interpreted(
        ...
        Err(RustCodeBlockError::TooComplex)
    }

    /// Проверяет синтаксис без выполнения
    fn check_syntax(&self, code: &str) -> Result<RustBlockResult, RustCodeBlockError> {
        let rustc = self.find_rustc()?;
        ...
        fs::write(&source_path, &full_code)
            .map_err(|e| RustCodeBlockError::IO(format!("Не удалось создать файл: {}", e)))?;
        ...
        if output.status.success() {
            ...
            Err(RustCodeBlockError::CompileError(format!(
    ...
    fn find_rustc(&self) -> Result<String, RustCodeBlockError> {
        ...
        // Проверяем наличие rustc
        let output = Command::new(rustc_name).arg("--version").output();

        match output {
            Ok(out) if out.status.success() => Ok(rustc_name.to_string()),
            _ => Err(RustCodeBlockError::NoRustCompiler),
        }
    }
...
///Ошибки связанные с раст вставками
#[derive(Debug, Clone)]
pub enum RustCodeBlockError {
    IO(String),
    CompileError(String),
    NoRustCompiler,
    ///Ошибка связана с невозможностью интерпретации раст вставки
    TooComplex,
}
impl RustCodeBlockError {
    pub fn message(&self) -> &str {
        match &self {
            RustCodeBlockError::IO(e) => e,
            RustCodeBlockError::NoRustCompiler => {
                "Компилятор Rust (rustc) не найден в системе. \
                 Установите Rust: https://rustup.rs/"
            }
            RustCodeBlockError::CompileError(e) => e,
            RustCodeBlockError::TooComplex => {
...
mod tests {
    ...
    fn test_interpret_println() {
        ...
        let result = executor.execute_interpreted(r#"println!("Hello, КуМир!");"#, &HashMap::new());
    ...
    fn test_interpret_with_variables() {
        ...
        let result = executor.execute_interpreted(r#"println!("x = {x}");"#, &vars);

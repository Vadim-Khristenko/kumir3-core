//! Семантическое версионирование для библиотек Kumir 3
//!
//! Поддерживает:
//! - Полные версии: `1.2.3`
//! - Требования: `>=1.0, <2.0`, `^1.5`, `~1.5.0`
//! - Предрелизные версии: `1.0.0-alpha.1`, `2.0.0-beta`
...
pub struct Version {
    ...
    pub patch: u32,
    /// Предрелизный тег (alpha, beta, rc.1)
...
impl Version {
    ...
    /// Создаёт версию с предрелизным тегом
    pub fn with_prerelease(mut self, pre: impl Into<String>) -> Self {
        self.prerelease = Some(pre.into());
        self
    }

    /// Создаёт версию с метаданными сборки
    pub fn with_build(mut self, build: impl Into<String>) -> Self {
        self.build_meta = Some(build.into());
        self
    }

    /// Проверяет, является ли это стабильной версией
    pub fn is_stable(&self) -> bool {
        self.prerelease.is_none() && self.major > 0
    }

    /// Проверяет, является ли это предрелизной версией
    pub fn is_prerelease(&self) -> bool {
        self.prerelease.is_some()
    }

    /// Проверяет совместимость с другой версией (^)
    /// ^1.2.3 == >=1.2.3, <2.0.0
...
impl Ord for Version {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        // Предрелизные версии меньше стабильных
...
/// Сравнивает предрелизные идентификаторы
...
impl FromStr for Version {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // Разбиваем на основную часть и метаданные сборки
        let (main, build_meta) = match s.split_once('+') {
            Some((m, b)) => (m, Some(b.to_string())),
            None => (s, None),
        };

        // Разбиваем на версию и предрелиз
        ...
        let major = parts.first()
            .ok_or_else(|| VersionParseError::new("отсутствует мажорная версия"))?
            .parse()
            .map_err(|_| VersionParseError::new("неверный формат мажорной версии"))?;

        let minor = parts.get(1)
            .map(|s| s.parse())
            .transpose()
            .map_err(|_| VersionParseError::new("неверный формат минорной версии"))?
            .unwrap_or(0);

        let patch = parts.get(2)
            .map(|s| s.parse())
            .transpose()
            .map_err(|_| VersionParseError::new("неверный формат патч версии"))?
            .unwrap_or(0);
...
impl fmt::Display for VersionParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Ошибка парсинга версии: {}", self.message)
    }
}
...
pub enum VersionOp {
    /// Точное совпадение: `=1.0.0`
    Exact,
    /// Больше: `>1.0.0`
    Greater,
    /// Больше или равно: `>=1.0.0`
    GreaterEq,
    /// Меньше: `<1.0.0`
    Less,
    /// Меньше или равно: `<=1.0.0`
    LessEq,
    /// Совместимые версии (cargo-style): `^1.0.0`
    Compatible,
    /// Близкие версии (npm-style): `~1.0.0`
    Tilde,
    /// Любая версия: `*`
    Wildcard,
}
...
impl VersionReq {
    /// Создаёт требование точной версии
    pub fn exact(version: Version) -> Self {
        Self { op: VersionOp::Exact, version }
    }

    /// Создаёт требование совместимой версии (^)
    pub fn compatible(version: Version) -> Self {
        Self { op: VersionOp::Compatible, version }
    }
...
/// Составной спецификатор версии (несколько требований через запятую)
/// Пример: `>=1.0, <2.0` или `^1.5`
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VersionSpec {
    /// Список требований (AND логика)
    pub requirements: Vec<VersionReq>,
}

impl VersionSpec {
    /// Создаёт спецификатор для любой версии
    pub fn any() -> Self {
        Self { requirements: vec![] }
    }

    /// Создаёт спецификатор для точной версии
    pub fn exact(version: Version) -> Self {
        Self {
    ...
    /// Создаёт спецификатор для совместимых версий
    pub fn compatible(version: Version) -> Self {
        Self {
    ...
    /// Проверяет, удовлетворяет ли версия всем требованиям
    pub fn matches(&self, version: &Version) -> bool {
...
impl FromStr for VersionSpec {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let requirements: Result<Vec<VersionReq>, _> = s
            .split(',')
...
mod tests {
    ...
    fn test_version_req() {
        ...
        let _req: VersionReq = ">=1.0, <2.0".parse().unwrap_or_else(|_| {
            VersionReq::compatible("1.0.0".parse().unwrap())
        });

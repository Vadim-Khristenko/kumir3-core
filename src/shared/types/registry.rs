...
pub type NativeFactory = Arc<
    dyn Fn(Vec<Value>) -> Result<Arc<dyn Any + Send + Sync>, String> + Send + Sync
>;
...
pub type StaticMethodHandler = Arc<
    dyn Fn(&str, Vec<Value>) -> Result<Value, String> + Send + Sync
>;

/// Обработчик доступа к полям.
/// Вызывается при `объект.поле`
pub type FieldAccessHandler = Arc<
    dyn Fn(&Arc<dyn Any + Send + Sync>, &str) -> Result<Value, String> + Send + Sync
>;

/// Обработчик установки полей.
/// Вызывается при `объект.поле := значение`
pub type FieldSetHandler = Arc<
    dyn Fn(&Arc<dyn Any + Send + Sync>, &str, Value) -> Result<(), String> + Send + Sync
>;
...
impl TypeRegistry {
    ...
    pub fn register_native_type(
        ...
        self.instance_handlers.write().unwrap().insert(type_id, instance_handler);
        self.static_handlers.write().unwrap().insert(type_id, static_handler);
    ...
    pub fn register_method(&self, method: MethodDef) {
        ...
        self.methods.write().unwrap().insert(full_name, method.clone());
    ...
    pub fn get_type_name(&self, type_id: TypeId) -> Option<String> {
        self.types.read().unwrap().get(&type_id).map(|t| t.name.clone())
    }
    ...
    pub fn create_instance(&self, type_id: TypeId, args: Vec<Value>) -> Result<Value, String> {
        ...
        if !type_def.is_instantiable {
            return Err(format!("Невозможно создать экземпляр типа '{}'", type_def.name));
        }
        
        // Если нативный тип — используем фабрику
        if type_def.is_native {
            let factory = self.factories.read().unwrap()
                .get(&type_id)
                .cloned()
            ...
            return Ok(Value::NativeObject { type_id, type_name, object });
        }
    ...
    pub fn call_static_method(
        ...
        let handler = self.static_handlers.read().unwrap()
            .get(&type_id)
            .cloned()
            .ok_or_else(|| {
                let name = self.get_type_name(type_id).unwrap_or_else(|| type_id.0.to_string());
    ...
    pub fn call_instance_method(
        ...
        let handler = self.instance_handlers.read().unwrap()
            .get(&type_id)
            .cloned()
            .ok_or_else(|| {
                let name = self.get_type_name(type_id).unwrap_or_else(|| type_id.0.to_string());
...
// Старые типы для совместимости с существующим интерпретатором
// Будут удалены после полного перехода

/// Старый TypeInfo (для совместимости)
#[derive(Debug, Clone)]
pub struct TypeInfo {
    pub id: TypeId,
    pub name: String,
...
/// Старый TypeKind (для совместимости)
#[derive(Debug, Clone)]
pub enum TypeKind {
    Primitive,
    KumirClass(super::class::ClassDef),
    Native(NativeType),
}

/// Старый NativeType (для совместимости)
#[derive(Debug, Clone)]
pub struct NativeType {
    pub module: String,
    pub static_methods: Vec<OldMethodInfo>,
    pub instance_methods: Vec<OldMethodInfo>,
    pub fields: Vec<OldFieldInfo>,
}

#[derive(Debug, Clone)]
pub struct OldMethodInfo {
    pub name: String,
    pub description: String,
    pub params: Vec<OldParamInfo>,
    pub return_type: Option<String>,
}

#[derive(Debug, Clone)]
pub struct OldParamInfo {
    pub name: String,
    pub type_name: String,
    pub optional: bool,
}
...
#[derive(Debug, Clone)]
pub struct OldFieldInfo {
    pub name: String,
    pub type_name: String,
    pub readonly: bool,
}

/// Старый InitMethod (для совместимости)
#[derive(Debug, Clone)]
pub enum InitMethod {
    Kumir(String),
    Native,

...
use crate::shared::types::{Token, Pattern, Value, Number};
...
impl Parser {
    ...
    pub fn parse_pattern(&mut self) -> ParseResult<Pattern> {
            ...
            Token::Integer(n) => {
                ...
                if self.match_token(&Token::DoubleDot) {
                    ...
                    return Ok(Pattern::Range {
                        start: Some(Box::new(crate::shared::types::Expr::Literal(
        ...
        if let Token::Identifier(name) = self.peek().clone() {
            ...
            if self.match_token(&Token::DoubleDot) {
                let end = self.parse_range_end()?;
                return Ok(Pattern::Range {
                    start: Some(Box::new(crate::shared::types::Expr::Variable(name))),
                    end,
                    inclusive: false,
                });
            }
    ...
    fn parse_range_end(&mut self) -> ParseResult<Option<Box<crate::shared::types::Expr>>> {
        // Может быть пусто (открытый диапазон)
        match self.peek() {
            Token::Integer(n) => {
                let n = *n;
                self.advance();
                Ok(Some(Box::new(crate::shared::types::Expr::Literal(
                    Value::Number(Number::I64(n))
                ))))
            }
            Token::Identifier(name) => {
                let name = name.clone();
                self.advance();
                Ok(Some(Box::new(crate::shared::types::Expr::Variable(name))))
            }
            _ => Ok(None),
        }
    }

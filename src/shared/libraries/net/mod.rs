...
use crate::shared::runtime::{global_runtime, init_global_runtime};
...
use crate::shared::types::library::{
    ...
    fn convert(v: serde_json::Value) -> Value {
        match v {
            ...
            serde_json::Value::Number(n) => {
                if let Some(i) = n.as_i64() { Value::Number(crate::shared::types::Number::from(i)) }
                else if let Some(f) = n.as_f64() { Value::Number(crate::shared::types::Number::from(f)) }
                else { Value::Null }
            }
    ...
    LibFunctionDef::new("http_запрос")
        ...
        .with_handler(|args| {
            ...
            rt.block_on(async {
                ...
                result.insert(Value::String("статус".to_string()), Value::Number(crate::shared::types::Number::from(response.status().code() as i64)));
                result.insert(Value::String("status".to_string()), Value::Number(crate::shared::types::Number::from(response.status().code() as i64)));
                Ok(Value::Map(result))
            })
        })
}
...
fn http_server_background_fn() -> LibFunctionDef {
    LibFunctionDef::new("http_сервер_фоновый")
        ...
        .with_handler(|args| {
            ...
            Ok(Value::Number(crate::shared::types::Number::from(port as i64)))
        })
}
...
fn const_status_ok() -> LibConstantDef {
    LibConstantDef { name: "HTTP_OK", aliases: &["СТАТУС_ОК"], const_type: TypeSpec::Int64, value: Value::Number(crate::shared::types::Number::from(200i64)), description: "HTTP 200 OK" }
}
fn const_status_not_found() -> LibConstantDef {
    LibConstantDef { name: "HTTP_NOT_FOUND", aliases: &["СТАТУС_НЕ_НАЙДЕНО"], const_type: TypeSpec::Int64, value: Value::Number(crate::shared::types::Number::from(404i64)), description: "HTTP 404 Not Found" }
}
fn const_status_error() -> LibConstantDef {
    LibConstantDef { name: "HTTP_ERROR", aliases: &["СТАТУС_ОШИБКА"], const_type: TypeSpec::Int64, value: Value::Number(crate::shared::types::Number::from(500i64)), description: "HTTP 500 Internal Error" }
}
fn const_port_http() -> LibConstantDef {
    LibConstantDef { name: "ПОРТ_HTTP", aliases: &["HTTP_PORT"], const_type: TypeSpec::Int64, value: Value::Number(crate::shared::types::Number::from(80i64)), description: "Стандартный порт HTTP" }
}
fn const_port_https() -> LibConstantDef {
    LibConstantDef { name: "ПОРТ_HTTPS", aliases: &["HTTPS_PORT"], const_type: TypeSpec::Int64, value: Value::Number(crate::shared::types::Number::from(443i64)), description: "Стандартный порт HTTPS" }
}

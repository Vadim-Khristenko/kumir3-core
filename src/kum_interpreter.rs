...
use kumir3_corelib::interpreter::{Interpreter, RuntimeError};
...
impl OutputLine {
    fn to_styled_line(&self) -> Line<'_> {
        match self {
            OutputLine::Normal(s) => Line::from(s.as_str()).style(Style::default().fg(Color::White)),
            OutputLine::Error(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Red)),
            OutputLine::Success(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Green)),
            OutputLine::Warning(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Yellow)),
            OutputLine::Input(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Yellow)),
            OutputLine::System(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Cyan)),
            OutputLine::Header(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Cyan).bold()),
            OutputLine::Code(s) => Line::from(s.as_str()).style(Style::default().fg(Color::Magenta)),
        }
    }
}
...
fn run_info_loop(terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<(), String> {
    loop {
        terminal.draw(|frame| draw_info(frame)).map_err(|e| e.to_string())?;
...
fn draw_info(frame: &mut Frame) {
    ...
    let title = Paragraph::new(vec![
        ...
        Line::from("║              KUMIR 3 - Интерпретатор                     ║").style(Style::default().fg(Color::Cyan).bold()),
        Line::from("╚══════════════════════════════════════════════════════════╝").style(Style::default().fg(Color::Cyan)),
    ]);
    ...
    let features = Paragraph::new(vec![
        Line::from(Span::styled("  Возможности:", Style::default().fg(Color::White).bold())),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Базовые типы: цел, вещ, лит, лог, сим")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Массивы и таблицы")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Алгоритмы с параметрами")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("ООП: классы, объекты, методы")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Перечисления и сопоставление с образцом")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Обработка исключений (попытка/перехват)")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Импорт модулей (.kum файлы)")]),
        Line::from(vec![Span::styled("    [+] ", Style::default().fg(Color::Green)), Span::raw("Стандартные библиотеки")]),
    ]);
    frame.render_widget(features, chunks[2]);
    
    // Использование
    let usage = Paragraph::new(vec![
        Line::from(Span::styled("  Использование:", Style::default().fg(Color::White).bold())),
        Line::from(vec![
            Span::styled("    kumir3-interpreter ", Style::default().fg(Color::Yellow)),
...
fn run_file_tui(path: &PathBuf, debug: bool, show_time: bool) -> Result<(), String> {
    ...
    app.output.push(OutputLine::Header(format!("  Файл: {}", path.display())));
    ...
    match result {
        Ok(value) => {
            if !matches!(value, kumir3_corelib::shared::types::Value::Null) {
                app.output.push(OutputLine::Success(format!("  Результат: {:?}", value)));
            } else {
                app.output.push(OutputLine::Success("  Выполнено успешно".to_string()));
            }
        }
        Err(e) => {
            app.error = Some(format!("{}", e));
            app.output.push(OutputLine::Error(format!("  Ошибка: {}", e)));
        }
    }
    
    if show_time {
        app.output.push(OutputLine::System(format!("  Время: {:.3} мс", elapsed.as_secs_f64() * 1000.0)));
    }
    
    app.output.push(OutputLine::System(String::new()));
    app.output.push(OutputLine::System("  Нажмите Enter или Q для выхода".to_string()));
    app.finished = true;
...
fn run_file_loop(terminal: &mut Terminal<CrosstermBackend<Stdout>>, app: &mut FileRunnerApp) -> Result<(), String> {
    loop {
        terminal.draw(|frame| draw_file_runner(frame, app)).map_err(|e| e.to_string())?;
            ...
            if let Event::Key(key) = event::read().map_err(|e| e.to_string())? {
                if key.kind == KeyEventKind::Press {
                    match key.code {
                        ...
                        KeyCode::Down | KeyCode::Char('j') => app.scroll = app.scroll.saturating_add(1).min(app.output.len().saturating_sub(1)),
                        KeyCode::PageUp => app.scroll = app.scroll.saturating_sub(10),
                        KeyCode::PageDown => app.scroll = app.scroll.saturating_add(10).min(app.output.len().saturating_sub(1)),
                        _ => {}
                    }
                }
            }
...
fn draw_file_runner(frame: &mut Frame, app: &FileRunnerApp) {
    ...
    let title = if app.error.is_some() { " Выполнение (ОШИБКА) " } else { " Выполнение " };
    let border_color = if app.error.is_some() { Color::Red } else { Color::Green };
...
fn run_check_tui(path: &PathBuf) -> Result<(), String> {
    ...
    output.push(OutputLine::Header(format!("  Проверка: {}", path.display())));
    ...
    match result {
        Ok(program) => {
            ...
            output.push(OutputLine::Normal(format!("  Алгоритмов: {}", program.algorithms.len())));
            output.push(OutputLine::Normal(format!("  Классов:    {}", program.classes.len())));
            output.push(OutputLine::Normal(format!("  Импортов:   {}", program.imports.len())));
            if program.main.is_some() {
                output.push(OutputLine::Success("  Главный алгоритм: определён".to_string()));
            }
        }
        Err(e) => {
            is_ok = false;
    ...
    output.push(OutputLine::System(String::new()));
    output.push(OutputLine::System(format!("  Время парсинга: {:.3} мс", elapsed.as_secs_f64() * 1000.0)));
    output.push(OutputLine::System(String::new()));
    output.push(OutputLine::System("  Нажмите Enter или Q для выхода".to_string()));
    
    let mut terminal = init_terminal()?;
    let result = run_simple_viewer(&mut terminal, &output, is_ok, if is_ok { " Синтаксис OK " } else { " Ошибка синтаксиса " });
    restore_terminal()?;
    result
}

fn run_simple_viewer(terminal: &mut Terminal<CrosstermBackend<Stdout>>, output: &[OutputLine], is_ok: bool, title: &str) -> Result<(), String> {
    loop {
        terminal.draw(|frame| {
            let area = frame.area();
            ...
            let items: Vec<ListItem> = output.iter()
                .map(|line| ListItem::new(line.to_styled_line()))
                .collect();
...
fn run_ast_tui(path: &PathBuf) -> Result<(), String> {
    ...
    if program.algorithms.is_empty() {
        ...
        for alg in &program.algorithms {
            let params: Vec<String> = alg.params.iter()
                .map(|p| format!("{}: {:?}", p.name, p.type_spec))
                .collect();
            let ret = alg.return_type.as_ref()
                .map(|t| format!(" -> {:?}", t))
                .unwrap_or_default();
            output.push(OutputLine::Success(format!("    алг {}({}){}", alg.name, params.join(", "), ret)));
        }
    }
    output.push(OutputLine::System(String::new()));
    ...
    if program.classes.is_empty() {
        ...
        for class in &program.classes {
            output.push(OutputLine::Success(format!(
                "    класс {} [{} полей, {} методов]",
                class.name, class.fields.len(), class.methods.len()
            )));
        }
    }
    output.push(OutputLine::System(String::new()));
    ...
    loop {
        terminal.draw(|frame| {
            ...
            let items: Vec<ListItem> = output[start..end].iter()
                .map(|line| ListItem::new(line.to_styled_line()))
                .collect();
            ...
            if let Event::Key(key) = event::read().map_err(|e| e.to_string())? {
                if key.kind == KeyEventKind::Press {
                    match key.code {
                        ...
                        KeyCode::Down | KeyCode::Char('j') => scroll = scroll.saturating_add(1).min(output.len().saturating_sub(1)),
                        KeyCode::PageUp => scroll = scroll.saturating_sub(10),
                        KeyCode::PageDown => scroll = scroll.saturating_add(10).min(output.len().saturating_sub(1)),
                        _ => {}
                    }
                }
            }
...
impl ReplApp {
    fn new(debug: bool) -> Self {
        ...
        app.output.push(OutputLine::Header("╔══════════════════════════════════════════════════════════════╗".to_string()));
        app.output.push(OutputLine::Header("║           KUMIR 3 - Интерактивный режим (TUI)                ║".to_string()));
        app.output.push(OutputLine::Header("╚══════════════════════════════════════════════════════════════╝".to_string()));
        app.output.push(OutputLine::System(String::new()));
        app.output.push(OutputLine::System("  Введите код на языке Кумир. Команды: .помощь .выход".to_string()));
        app.output.push(OutputLine::System("  Ctrl+D выход | Page Up/Down прокрутка".to_string()));
        app.output.push(OutputLine::System(String::new()));
    ...
    fn execute(&mut self) {
        let input = if self.multiline {
            ...
            if trimmed == "кон" || trimmed == "все" || trimmed == "кц" {
                ...
                if closes >= opens {
                    ...
                    self.output.push(OutputLine::Input(format!("...   {}", self.input)));
                    self.input.clear();
                    self.cursor_pos = 0;
                    return;
                }
        ...
        self.output.push(OutputLine::Input(format!("кумир> {}", input)));
        
        let trimmed = input.trim();
        if trimmed.ends_with("нач") || trimmed.ends_with("то") || 
           trimmed.ends_with("иначе") || trimmed.ends_with("нц") ||
        ...
        match trimmed {
            ...
            ".debug" | ".отладка" => {
                ...
                let msg = if self.debug_mode { "включён" } else { "выключен" };
                self.output.push(OutputLine::System(format!("  Режим отладки: {}", msg)));
                return;
            }
            ".vars" | ".переменные" => {
                self.output.push(OutputLine::System("  Переменные: (пока не реализовано)".to_string()));
                return;
            }
            ":q!" | ":q" | ":wq" | ":x" => {
                self.output.push(OutputLine::Warning("  Мы не в Vim, к сожалению.".to_string()));
                self.output.push(OutputLine::Warning("  Но автор оценил ваше стремление к оптимизации).".to_string()));
                self.output.push(OutputLine::Warning("  Используйте .выход для выхода из консоли.".to_string()));
                return;
            }
            ".ai" | ".vibecode" | ".vibecoding" => {
                self.output.push(OutputLine::Error("  Какой ВАЙБ-КОДИНГ в REPL!?".to_string()));
                self.output.push(OutputLine::Error("  Пожалуйста, используйте нормальные команды и не позорьтесь.".to_string()));
                return;
            }
            ".whoareyou" => {
                self.output.push(OutputLine::System("  Я - интерактивная консоль Кумир 3.".to_string()));
                self.output.push(OutputLine::System("  Моя задача - помочь вам выполнять код на языке Кумир.".to_string()));
                self.output.push(OutputLine::System("  Пожалуйста, используйте меня по назначению.".to_string()));
                return;
            }
            _ if trimmed.starts_with('.') => {
                self.output.push(OutputLine::Error(format!("  Неизвестная команда: {}", trimmed)));
                return;
            }
            _ => {}
        }
        ...
        match self.interpreter.run(&input) {
            Ok(value) => {
                ...
                if !matches!(value, kumir3_corelib::shared::types::Value::Null) {
                    self.output.push(OutputLine::Success(format!("  => {:?}", value)));
                }
            }
            Err(e) => {
                self.output.push(OutputLine::Error(format!("  Ошибка: {}", e)));
            }
        }
        ...
        match (code, mods) {
            ...
            (KeyCode::Char('c'), KeyModifiers::CONTROL) => {
                if self.multiline {
                    ...
                    self.output.push(OutputLine::Warning("  Ввод отменён".to_string()));
                }
                self.input.clear();
                self.cursor_pos = 0;
            }
            ...
            (KeyCode::PageDown, _) => self.scroll = self.scroll.saturating_add(5).min(self.output.len().saturating_sub(1)),
            (KeyCode::Left, _) => self.cursor_pos = self.cursor_pos.saturating_sub(1),
            (KeyCode::Right, _) => if self.cursor_pos < self.input.len() { self.cursor_pos += 1 },
            (KeyCode::Home, _) => self.cursor_pos = 0,
            (KeyCode::End, _) => self.cursor_pos = self.input.len(),
            (KeyCode::Backspace, _) => if self.cursor_pos > 0 { self.input.remove(self.cursor_pos - 1); self.cursor_pos -= 1; },
            (KeyCode::Delete, _) => if self.cursor_pos < self.input.len() { self.input.remove(self.cursor_pos); },
            (KeyCode::Char(c), KeyModifiers::NONE | KeyModifiers::SHIFT) => { self.input.insert(self.cursor_pos, c); self.cursor_pos += 1; }
            _ => {}
        }
    }
}
...
fn run_repl_tui(debug: bool) -> Result<(), String> {
    ...
    loop {
        terminal.draw(|frame| draw_repl(frame, &app)).map_err(|e| e.to_string())?;
...
fn draw_repl(frame: &mut Frame, app: &ReplApp) {
    ...
    let items: Vec<ListItem> = app.output.get(start..end)
        .unwrap_or(&[])
        .iter()
        .map(|line| ListItem::new(line.to_styled_line()))
        .collect();
    ...
    if total > visible {
        let scrollbar = Scrollbar::new(ScrollbarOrientation::VerticalRight)
            ...
            .end_symbol(Some("▼"));
        let mut state = ScrollbarState::new(total).position(app.scroll);
        frame.render_stateful_widget(scrollbar, chunks[0].inner(Margin { horizontal: 0, vertical: 1 }), &mut state);
    }
    
    // Ввод
    let prompt = if app.multiline { "...   " } else { "кумир> " };
    let input_text = format!("{}{}", prompt, app.input);
    
    let title = if app.debug_mode { " Ввод [отладка] " } else { " Ввод " };
    let border_color = if app.multiline { Color::Yellow } else { Color::Green };
